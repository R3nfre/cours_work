...
public class Deal {
    ...
    private static final void roldqo32(int rot, int[] ki, int ioff, int[] ko, int ooff) {
    ...
    public final byte lRot8(byte v, int rot) {
        return (byte) ((v << rot) | ((v & 0xff) >>> (8 - rot)));
    }

    public final int sbox2(int x) {
        return (int) (lRot8(SBOX1[x], 1) & MASK8);
    }

    public final int sbox3(int x) {
        return (int) (lRot8(SBOX1[x], 7) & MASK8);
    }

    public final int sbox4(int x) {
        return (int) (SBOX1[((int) lRot8((byte) x, 1) & MASK8)] & MASK8);
    }

    private final void dealF2(int[] s, int[] skey, int keyoff) {
        int t1, t2, u, v;
    ...
    private final void dealFLs(int[] s, int[] fkey, int keyoff) {
    ...
    private final int processBlock128(byte[] in, int inOff, byte[] out, int outOff) {
        ...
        dealF2(state, subkey, 0);
        dealF2(state, subkey, 4);
        dealF2(state, subkey, 8);
        dealFLs(state, ke, 0);
        dealF2(state, subkey, 12);
        dealF2(state, subkey, 16);
        dealF2(state, subkey, 20);
        dealFLs(state, ke, 4);
        dealF2(state, subkey, 24);
        dealF2(state, subkey, 28);
        dealF2(state, subkey, 32);
    ...
    private final int processBlock192or256(byte[] in, int inOff, byte[] out, int outOff) {
        for (int i = 0; i < 4; i++) {
            state[i] = bytes2int(in, inOff + (i * 4));
            state[i] ^= kw[i];
        }
        ...
        dealF2(state, subkey, 32);
        dealFLs(state, ke, 8);
        dealF2(state, subkey, 36);
        dealF2(state, subkey, 40);
        dealF2(state, subkey, 44);
    ...
    public int processBlock(byte[] in, int inOff, byte[] out, int outOff) throws IllegalStateException {
        ...
        if (!initialized) {
            throw new IllegalStateException("Deal is not initialized");
        }
        if (_keyis128) {
